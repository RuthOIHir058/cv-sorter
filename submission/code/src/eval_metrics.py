...
def run_evaluation(gold_file="tests/gold_standard.json"):
    ...
    gold_relevant = [r["id"] for r in gold["resumes"] if r.get("relevant")]
    ...
    with open("results/ranking_with_gold.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["candidate_id", "gold_rank", "system_rank"])
        for cid in gold_ranked_ids:
            writer.writerow([
                cid,
                gold_ranked_ids.index(cid) + 1,
                system_ranked_ids.index(cid) + 1 if cid in system_ranked_ids else "NA"
            ])
